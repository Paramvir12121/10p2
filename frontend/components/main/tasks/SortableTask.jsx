...
export function SortableTask({ 
  ...
  const taskClassNames = cn(
    'flex flex-col gap-0.5 py-0.5 px-1 rounded-sm border text-[13px] min-h-[22px] w-full', // Increased from text-[11px]
  ...
  return (
    ...
    >
      <div className="flex items-center gap-1">
        ...
        <span 
          className={`flex-grow text-[13px] leading-tight truncate ${completed ? 'line-through text-slate-400' : isOverdue ? 'text-red-600 dark:text-red-400' : 'text-slate-800 dark:text-slate-200'} transition-all duration-300`} // Increased from text-[11px]
        >
          {text}
        </span>
      ...
      {showNotes && notes && (
        <div className="ml-4 mt-0.5 text-[11px] text-muted-foreground bg-muted/20 p-1 rounded">
          {notes}
        </div>
      )}
    </div>
